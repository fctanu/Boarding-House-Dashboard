...
const CSVImporter: React.FC<CSVImporterProps> = ({ onImport }) => {
  ...
  const parseCSV = (csvText: string): Omit<Tenant, 'id' | 'status'>[] => {
    ...
    return rows.map((row, index) => {
      ...
      if (!name || !roomNumberStr || !rentAmountStr || !dueDateStr) {
        throw new Error(
          `Invalid or missing data on row ${index + 2}. Please check all values are present.`,
        );
      }
      ...
      if (isNaN(roomNumber) || isNaN(rentAmount)) {
        throw new Error(
          `Invalid number format on row ${index + 2}. Check room number and rent amount.`,
        );
      }
  ...
  const parseJSON = (jsonText: string): Omit<Tenant, 'id' | 'status'>[] => {
    ...
    return data.map((item, index) => {
      ...
      if (
        ...
        typeof dueDateStr !== 'string'
      ) {
        throw new Error(
          `Invalid data type in object at index ${index}. Check that name/dueDate are strings and roomNumber/rentAmount are numbers.`,
        );
      }
  ...
  const CSVInstructions = () => (
    <>
      ...
      <code className="bg-slate-200 text-slate-800 px-2 py-1 rounded text-sm">
        name,roomNumber,rentAmount,dueDate
      </code>
      <ul className="list-disc list-inside text-sm text-slate-600 mt-2 space-y-1">
        <li>
          <span className="font-semibold">name:</span> Tenant&#39;s full name (e.g., John Doe)
        </li>
        <li>
          <span className="font-semibold">roomNumber:</span> The assigned room number (e.g., 101)
        </li>
        <li>
          <span className="font-semibold">rentAmount:</span> The monthly rent, numbers only (e.g.,
          550.00)
        </li>
        <li>
          <span className="font-semibold">dueDate:</span> The next due date in YYYY-MM-DD or
          DD/MM/YYYY format
        </li>
      </ul>
    </>
          ...
          {`[
  {
    ...
    "dueDate": "2024-10-01"
      ...
      <ul className="list-disc list-inside text-sm text-slate-600 mt-2 space-y-1">
        <li>
          <span className="font-semibold">name:</span> Must be a{' '}
          <code className="text-xs">string</code>.
        </li>
        <li>
          <span className="font-semibold">roomNumber:</span> Must be a{' '}
          <code className="text-xs">number</code>.
        </li>
        <li>
          <span className="font-semibold">rentAmount:</span> Must be a{' '}
          <code className="text-xs">number</code>.
        </li>
        <li>
          <span className="font-semibold">dueDate:</span> Must be a{' '}
          <code className="text-xs">string</code> in YYYY-MM-DD or DD/MM/YYYY format.
        </li>
      </ul>
    </>
  );
  ...
  return (
    <div className="bg-white p-8 rounded-xl shadow-md max-w-2xl mx-auto">
      ...
      <p className="text-slate-500 mb-6">
        Upload a CSV or JSON file to add multiple tenants at once.
      </p>
