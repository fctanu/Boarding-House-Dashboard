...
const UndoPaymentButton: React.FC<UndoPaymentButtonProps> = ({
  ...
  return confirm ? (
    <button
      onClick={() => {
        onUndo(tenantId, PaymentStatus.Unpaid);
        setConfirm(false);
      }}
      className="px-3 py-1 bg-rose-500 text-white rounded-lg text-sm font-semibold hover:bg-rose-600 transition-colors"
    >
      Confirm Undo
    </button>
  ) : (
    <button
      ...
      className="px-3 py-1 bg-slate-200 text-slate-700 rounded-lg text-sm font-semibold hover:bg-slate-300 transition-colors"
    >
      Undo
    </button>
  );
...
const StatusBadge: React.FC<{ status: PaymentStatus }> = ({ status }) => {
  ...
  switch (status) {
    ...
    case PaymentStatus.Overdue:
      return (
        <span className={`${baseClasses} bg-rose-100 text-rose-700`}>
